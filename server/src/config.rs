...
impl ServerConfig {
  pub fn from_env() -> anyhow::Result<Self> {
    ...
    let http_port = read_env("AGENT_HUB_HTTP_PORT").unwrap_or_else(|| "42001".into());
    let ws_port = read_env("AGENT_HUB_WS_PORT").unwrap_or_else(|| "42002".into());
    ...
    let prompt_dir =
      PathBuf::from(read_env("AGENT_HUB_PROMPT_PROFILE_DIR").unwrap_or_else(|| "prompts/profiles".into()));
