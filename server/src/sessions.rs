...
impl SessionStore {
  ...
  pub async fn subscribe(
    &self,
    session_id: &str,
  ) -> Option<broadcast::Receiver<WsEvent>> {
    let sender = {
      let mut channels = self.channels.write().await;
      channels
        .entry(session_id.to_string())
  ...
  pub async fn messages(&self, session_id: &str) -> Option<Vec<SessionMessage>> {
    let sessions = self.sessions.read().await;
    sessions.get(session_id).map(|session| session.messages.clone())
  }
}
...
pub enum WsEvent {
  ...
  AssistantMessageStart { session_id: String, message_id: String },
  AssistantMessageChunk {
    session_id: String,
    message_id: String,
    text_chunk: String,
  },
  AssistantMessageComplete { session_id: String, message_id: String },
